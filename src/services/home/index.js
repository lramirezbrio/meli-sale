...
var getCategories = function(){
    meliObject.get('sites/' + config.config.site_id +'/categories', function (err, res) {
        getItemsCategories(res);
    });
};

var getItemsCategories = function(categorias){
    categorias.forEach(categoria => {
        meliObject.get('sites/' + config.config.site_id +'/search?category=' + categoria.id+ '&shipping=free&discount=40-100', function (err, res) {
